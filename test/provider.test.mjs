...
function sign({method, rawPath, body = Buffer.alloc(0), contentType, jti = randomUUID()}) {
...
function send(port, {method, rawPath, body = Buffer.alloc(0), contentType, token, adapter = ADAPTER}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const request = httpRequest({host: "127.0.0.1", port, method, path: rawPath, headers}, (response) => {
      const chunks = [];
...
async function fixture(run) {
  ...
  await writeFile(path.join(storageRoot, "contracts", "sample document.docx"), "original", "utf8");
...
test("serves the complete signed storage lifecycle", async () => fixture(async ({storageRoot, port}) => {
  const encodedFile = "contracts/sample%20document.docx";

  const info = await send(port, {method: "GET", rawPath: `/tfo-storage/v1/${encodedFile}/info`});
  ...
  const list = await send(port, {method: "GET", rawPath: "/tfo-storage/v1/contracts/list"});
  assert.equal(list.status, 200);
  assert.equal(JSON.parse(list.body).entries[0].path, "contracts/sample document.docx");

  const get = await send(port, {method: "GET", rawPath: `/tfo-storage/v1/${encodedFile}/get`});
  ...
  assert.equal(await readFile(path.join(storageRoot, "contracts", "sample document.docx"), "utf8"), "saved-document");
...
test("rejects replayed and body-mismatched requests before storage access", async () => fixture(async ({storageRoot, port}) => {
  const rawPath = "/tfo-storage/v1/contracts/sample%20document.docx/info";
  const token = sign({method: "GET", rawPath});
  assert.equal((await send(port, {method: "GET", rawPath, token})).status, 200);
  assert.equal((await send(port, {method: "GET", rawPath, token})).status, 401);
  ...
  await assert.rejects(readFile(path.join(storageRoot, "contracts", "forged.docx")), {code: "ENOENT"});
}));

test("contains paths and refuses symbolic links and root deletion", async () => fixture(async ({storageRoot, port}) => {
  const outside = await mkdtemp(path.join(os.tmpdir(), "tfo-provider-outside-"));
  try {
...
test("health endpoint does not expose configuration", async () => fixture(async ({port}) => {
  const response = await new Promise((resolve, reject) => {
    const request = httpRequest({host: "127.0.0.1", port, method: "GET", path: "/healthz"}, (result) => {
      const chunks = [];
