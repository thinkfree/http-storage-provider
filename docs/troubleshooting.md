...
| `put` returns `400`, `411`, or `413` | Compare fixed `Content-Length`, received bytes, Provider size limit, and available staging space. | Send the complete body with fixed length, increase an intentional Provider limit, or free staging space. Do not enable chunked PUT. |
...
| INFO, LIST, or GET reports a response framing error | Inspect the successful response headers and compare bytes actually sent. | Send exactly one decimal `Content-Length`, the required media type, no `Transfer-Encoding` or `Content-Encoding`, and exactly that many body bytes. Keep large GET bodies streamed. |
