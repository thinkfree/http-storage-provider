...
if (!adapter || !secret) throw new Error("Run npm run init before the smoke test");
...
async function signedList(rawPath) {
  ...
  const claims = encode({
    ...
    request: {
      ...
      path: rawPath,
      content_length: 0,
      content_sha256: createHash("sha256").update(Buffer.alloc(0)).digest("hex"),
    },
  });
  ...
  if (!response.ok) throw new Error(`List failed with HTTP ${response.status}: ${await response.text()}`);
...
for (const expected of ["Welcome.txt", "samples"]) {
  if (!rootNames.includes(expected)) throw new Error(`Root list did not contain ${expected}`);
}
const sampleNames = await signedList("/tfo-storage/v1/samples/list");
for (const expected of ["sample.docx", "sample.xlsx", "sample.pptx"]) {
  if (!sampleNames.includes(expected)) throw new Error(`Sample list did not contain ${expected}`);
}
